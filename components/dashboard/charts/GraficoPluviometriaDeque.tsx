...
function barColor(mm: number): string {
  if (mm === 0) return "hsl(210, 20%, 82%)"
  if (mm < 50)  return "hsl(210, 80%, 68%)"
  if (mm < 120) return "hsl(217, 91%, 55%)"
  return          "hsl(225, 80%, 42%)"
}
...
function CustomTooltip({ active, payload }: any) {
  ...
  const label =
    value === 0 ? "Sem registros de chuva"
    : value < 50 ? "Precipitação baixa"
...
export function GraficoPluviometriaDeque() {
  const [raw, setRaw] = useState<{ periodo: string; pluviometria: number }[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    fetch("/api/deque-pedras/historico/pluviometria")
      .then((r) => r.json())
      .then((json) => {
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <CardDescription className="text-xs mt-0.5">
                Deque de Pedras · Acumulado mensal · Jan/2024–{new Date().getFullYear()}
              </CardDescription>
            </div>
          </div>
          {stats && (
